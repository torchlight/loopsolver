...
public class Phase1WD {
	...
	private static final int N_ROW = 44;
	private static final int N_ROW2 = 832;
	private static final int N_ROW4 = 40600;
	...
	public static void initialise() {
		...
		// fill `decodeRow` with an "obviously" invalid value like -1.
		for (int i = 0, k = 0; i < 1024; i++) {
			...
			if (a[0] < a[1] || a[1] < a[2] || a[2] < a[3] || a[3] < a[4]) {
				...
				encodeRow[i] = encodeRow[ii];
				//System.out.printf("%d: %d %d %d %d %d\n", encodeRow[i], a[0], a[1], a[2], a[3], a[4]);
			}
			else {
				encodeRow[i] = k;
				decodeRow[k] = i;
		...
		for (int i = 0, k = 0; i < N_ROW2; i++) {
			for (int j = 0; j < N_ROW2; j++) {
				...
				// we require the decoded output to always have sorted rows
				//System.out.printf("%d: (%d, %d)\n", k, i, j);
				k++;
			}
		}
